...
If the input data is too sparse (e.g., missing fundamentals AND no recent news), return a thesis with \`summary: "Insufficient data for thesis generation."\` and empty arrays.`;

const RETRY_SUFFIX = `\n\nCRITICAL: Your previous response failed schema validation. Ensure ALL evidence fields cite specific numbers from the JSON (e.g. "P/E of 24.3"). NEVER write vague phrases like "strong fundamentals", "good management", or comparisons to data not in the JSON.`;
